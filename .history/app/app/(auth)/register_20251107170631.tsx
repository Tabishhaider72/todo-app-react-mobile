...
export default function RegisterScreen(){
  ...
  const onRegister = async () => {
    try {
      const res = await axios.post('http://<YOUR_DEV_HOST>:4000/api/auth/register', { email, password });
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.switch} onPress={()=>router.push('/(auth)/login')}>Have account? Log in</Text>
    </View>
  );
}
